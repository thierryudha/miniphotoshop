...
rgba_img = np.dstack([img, np.full(img.shape[:2], 128, dtype=np.uint8)])
rgba_hist = ip.compute_histograms(rgba_img)
assert ip.select_histogram_channels(rgba_hist, 'all') == ('R', 'G', 'B', 'A')
assert rgba_hist['A'].shape == (256,)
